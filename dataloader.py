...
class RepeatSequenceDataset(Dataset):
    ...
    def forward_strand(self, index):
        ...
        repeats_in_sequence = anno_df.loc[
            ...
            #             -----------------------
            #             ^rep_start            ^rep_end
            | ((anno_df['start'] < end) & (end < anno_df['end']) & (anno_df["start"] < anno_df["end"]))
            #            ----------------------------
            #            ^seq_start                  ^seq_end 
            #-----------------------
            #^rep_start            ^rep_end
            | ((anno_df['start'] < start) & (start < anno_df['end']) & (anno_df["start"] < anno_df["end"]))
        ]
        #truncate repeat pos 
        repeats_in_sequence = repeats_in_sequence.apply(
                lambda x: [max(start, x["start"]), min(end, x["end"]), x["subtype"]], axis=1, result_type='broadcast')
